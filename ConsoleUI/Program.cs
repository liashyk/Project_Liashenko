...
// ════════════════════════════════════════════════════════════════════
//  Лабораторна робота №5 — Збереження даних (JSON/XML) + IDisposable
// ════════════════════════════════════════════════════════════════════

PrintHeader("Лабораторна робота №5 — JSON, XML, IDisposable");

using (var storage = new MovieStorage("storage.log"))
{
    // Якщо є старі дані — завантажуємо
    if (File.Exists("movies.json"))
    {
        storage.LoadFromJson("movies.json");
        Console.WriteLine($"  Завантажено {storage.Count} фільмів з JSON.");
    }
    else
    {
        // Додаємо фільми з попередніх лабораторних
        foreach (var m in movies)   // 'movies' — список з ЛР2
            storage.AddMovie(m);

        Console.WriteLine($"  Додано {storage.Count} фільмів у сховище.");
    }

    // Зберігаємо
    storage.SaveToJson("movies.json");
    storage.ExportToXml("movies_high_rated.xml");

    Console.WriteLine("  Файли успішно створено:");
    Console.WriteLine("     ✓ movies.json");
    Console.WriteLine("     ✓ movies_high_rated.xml");
    Console.WriteLine("     ✓ storage.log");
}
Console.WriteLine("Файли створено тут: " + Directory.GetCurrentDirectory());

Console.WriteLine(new string('═', 72));
Console.WriteLine("  Лабораторна робота №5 виконана успішно!");
Console.WriteLine(new string('═', 72));
